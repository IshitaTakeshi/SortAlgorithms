...
Dictionary *append_dictionary(Dictionary *dictionary, int key, int value) {
    Map *new_maps = dictionary->maps;

    new_maps = (Map *)realloc(dictionary->maps, (dictionary->size+1)*sizeof(Map));
    new_maps[dictionary->size] = init_map(key, value);
    dictionary->maps = new_maps; 
    dictionary->size += 1;
    return dictionary;
}
